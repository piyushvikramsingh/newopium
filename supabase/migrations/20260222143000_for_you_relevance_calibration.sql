-- For You relevance calibration: rebalance recency, affinity, and popularity
...
scored AS (
  SELECT
    v.id AS video_id,
    (
      (COALESCE(v.likes_count, 0) * 1.1) +
      (COALESCE(v.comments_count, 0) * 1.6) +
      (COALESCE(v.shares_count, 0) * 2.2) +
      (22 / SQRT(GREATEST(1::DOUBLE PRECISION, EXTRACT(EPOCH FROM (NOW() - v.created_at)) / 3600::DOUBLE PRECISION))) +
      (COALESCE(es.affinity, 0) * 2.6) +
      (CASE WHEN EXISTS (SELECT 1 FROM followed f WHERE f.following_id = v.user_id) THEN 9 ELSE 0 END) +
      (
        COALESCE((
          SELECT COUNT(*)
          FROM interests i
